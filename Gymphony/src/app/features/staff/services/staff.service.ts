...
export class StaffService {
  ...
  public getAllAdmins(): Observable<User[]> {
    return this.http.get<User[]>(`${this.config.apiUrl}/admins`)
      .pipe(catchError((error: HttpErrorResponse) => this.handlerError(error)));
  }

  public getAllStaff(): Observable<Staff[]> {
    return this.http.get<Staff[]>(`${this.config.apiUrl}/staff`)
      .pipe(catchError((error: HttpErrorResponse) => this.handlerError(error)));
  }

  public addAdmin(admin: SignUpDetails): Observable<User> {
    return this.http.post<User>(`${this.config.apiUrl}/admins/sign-up`, admin)
      .pipe(catchError((error: HttpErrorResponse) => this.handlerError(error)));
  }

  public addStaff(staff: FormData): Observable<Staff> {
    return this.http.post<Staff>(`${this.config.apiUrl}/staff/sign-up`, staff)
      .pipe(catchError((error: HttpErrorResponse) => this.handlerError(error)));
  }
  ...
  public unblockAdmin(adminId: string): Observable<object> {
    return this.http.put(`${this.config.apiUrl}/admins/unblock/${adminId}`, null)
      .pipe(catchError((error: HttpErrorResponse) => this.handlerError(error)));
  }

  public removeAdmin(adminId: string) {
    return this.http.delete(`${this.config.apiUrl}/admins/${adminId}`)
      .pipe(catchError((error: HttpErrorResponse) => this.handlerError(error)));
  }

  public removeStaff(staffId: string) {
    return this.http.delete(`${this.config.apiUrl}/staff/${staffId}`)
      .pipe(catchError((error: HttpErrorResponse) => this.handlerError(error)));
  }
